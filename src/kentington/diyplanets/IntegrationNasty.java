...
public class IntegrationNasty extends BaseIndustry implements MarketImmigrationModifier {
	...
	static float TERRAFORM_TIME=180f;
	...
	public String getBuildOrUpgradeProgressText() {
		...
		return "Subjugating: " + left + " " + days + " left";
	}
	...
	public void advance(float amount) {
		...
		if (!disrupted) {
			...
			else if(this.getMaxDeficit(Commodities.HAND_WEAPONS).two < 2)
			{
				if(this.getMaxDeficit(Commodities.HAND_WEAPONS).two > 0)
					days *= 0.125f;
				buildProgress += days;
				if (Commodities.ALPHA_CORE.equals(aiCoreId)) 
				{
				...
				market.getHazard().modifyFlat("Integration",hazardReduction,"Subjugation Corps");
				
				market.getStability().modifyFlat("Integration", hazardReduction * -8,"Subjugation Corps");
			}
	...
	public void modifyIncoming(MarketAPI market, PopulationComposition incoming) {
		...
		incoming.getWeight().modifyFlat(getModId(), market.getSize(), Misc.ucFirst("Subjugation Corps"));
	}
