...
public class WaterReceiver extends BaseIndustry {
	...
	public boolean isAvailableToBuild() {
		...
		try
		{
			...
			if (planetCat.equals("cat_hab2") && !market.hasCondition("hydrated"))
			{
				List<PlanetAPI> otherPlanets=market.getPlanetEntity().getContainingLocation().getPlanets();
				for(int i=0;i<otherPlanets.size();i++)
				{
					PlanetAPI planet = otherPlanets.get(i);
					MarketAPI market = planet.getMarket();
					if(market!=null && market.hasIndustry("waterlauncher"))
						return true;
				}
			}
		} catch (Exception e) {
	...
	public void finishBuildingOrUpgrading() {
		...
		if (building) {
			...
			market.addCondition("hydrated");
			market.removeIndustry("waterreceiver", null, false);
