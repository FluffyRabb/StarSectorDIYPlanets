...
public class IntegrationNice extends BaseIndustry implements MarketImmigrationModifier {
	...
	public String getBuildOrUpgradeProgressText() {
		...
		return "Integrating: " + left + " " + days + " left";
	}
	...
	public void advance(float amount) {
		...
		if (!disrupted) {
			...
			else if(this.getMaxDeficit(Commodities.FOOD).two < 2)
			{
				if(this.getMaxDeficit(Commodities.FOOD).two > 0)
					days *= 0.125f;
				buildProgress += days;
				if (Commodities.ALPHA_CORE.equals(aiCoreId)) 
				{
				...
				market.getHazard().modifyFlat("Integration",hazardReduction,"Integration Corps");
				
				market.getStability().modifyFlat("Integration", hazardReduction * -8,"Integration Corps");
			}
	...
	public void modifyIncoming(MarketAPI market, PopulationComposition incoming) {
		...
		incoming.getWeight().modifyFlat(getModId(), market.getSize(), Misc.ucFirst("Integration Corps"));
	}
